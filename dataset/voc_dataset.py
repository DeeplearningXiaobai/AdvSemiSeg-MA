...
class VOCDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(416, 416), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255):
        self.root = root
        ...
        for name in self.img_ids:
            ...
            label_file = osp.join(self.root, "SegmentationClass/%s" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class VOCDataTestSet(data.Dataset):
    def __init__(self, root, list_path, crop_size=(416, 416), mean=(128, 128, 128)):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        self.mean = mean
